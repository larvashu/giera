...
func _build_color_layer(color_key: String, entries: Array, height_resolver: Callable, width: float, height: float) -> void:
	...
	grass.set("color_variation_strength", 0.12)
	grass.set("dry_tint", Color(0.76, 0.70, 0.48))
	grass.set("fresh_tint", Color(0.62, 0.78, 0.43))
	grass.set("variation_scale", 24.0)
	grass.set("albedo", Color.WHITE)
	grass.set("interactive", false)
	grass.set("light_mode", 0)
	grass.set("grass_strength", 0.48)
	grass.set("roughness", 0.96)
	grass.set("specular", 0.12)
	grass.set("alpha_scissor_threshold", 0.46)
	grass.set("optimization_by_distance", true)
	grass.set("optimization_level", 9.0)
	grass.set("optimization_dist_min", 70.0)
	grass.set("optimization_dist_max", 520.0)
